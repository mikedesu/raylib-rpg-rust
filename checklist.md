# checklist.md

1. [x] create the basic setup of
    - [x] create window at 1280x720
    - [x] while window should not close, do 
        - [x] clear screen black 
        - [x] draw hello world text white in the center of the window
            - [x] default font face
            - [x] font size 20
2. [x] create a render texture of 640x360 after creating the window, and
    - [x] modify the main loop to draw to the texture instead of the window
    - [x] modify the end of the main loop to draw the texture to the window 
        - [x] the texture must be scaled up to the size of the window
3. [ ] change the text from "hello world" to "evildojo666"
